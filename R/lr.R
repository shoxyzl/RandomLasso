...
lr <- function(x,y,lambda=c("min","1st"),continuous=T){
  if(continuous){
    ...
    if(lambda=='min'){
      fit <- glmnet::glmnet(data.matrix(x),y.factor, alpha = 1,family="binomial",lambda = cv.lambda$lambda.min)
    }
    else{
      fit <- glmnet::glmnet(data.matrix(x),y.factor, alpha = 1,family="binomial",lambda = cv.lambda$lambda.1st)
    }
  }
  c <- coef(fit)
